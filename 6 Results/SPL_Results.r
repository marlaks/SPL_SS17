...
ggplot(accuracy.results, aes(x = iteration)) + geom_line(aes(y = dt.results.accuracy, colour = "dt.results.accuracy")) + 
    ...
    ylab("accuracy percentage") + ggtitle("Accuracy of each model over multiple iterations") + theme(panel.background = element_rect(fill = "transparent"))
...
ggplot(precision.results, aes(x = iteration)) + geom_line(aes(y = dt.results.precision, colour = "dt.results.precision")) + 
    ...
    ylab("precision") + ggtitle("precision of each model over multiple iterations") + theme(panel.background = element_rect(fill = "transparent"))
...
ggplot(recall.results, aes(x = iteration)) + geom_line(aes(y = dt.results.recall, colour = "dt.results.recall")) + 
    ...
    ylab("recall") + ggtitle("recall of each model over multiple iterations") + theme(panel.background = element_rect(fill = "transparent"))
