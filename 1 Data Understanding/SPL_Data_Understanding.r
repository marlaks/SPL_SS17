...
# are there errors concerning previous campaigns? (yes in 4110 rows variables contradict each
# other about whether a previous campaign existed)
errorCount = ifelse(bank$`outcome of previous campaign` == "nonexistent" & bank$`days since last contacted from previous campaign` != 
    999 | bank$`outcome of previous campaign` != "nonexistent" & bank$`days since last contacted from previous campaign` == 
    999, "error", "no error")
...
# treat 999 error through binning what are the frequencies of the values?
table(bank$`days since last contacted from previous campaign`)
# bin into 0-2, 3-5, 6-8, 9-11, 12-14, >= 15, never contacted
bank$`days since last contacted from previous campaign` = cut(bank$`days since last contacted from previous campaign`, 
    breaks = c(0, 3, 6, 9, 12, 15, 999, 1000), labels = c("0-2", "3-5", "6-8", "9-11", "12-14", 
        "15+", "never contacted before"), right = FALSE)
